return {
  ...
  config = function()
    ...
    vim.keymap.set("n", "<leader>to", function() nt.output.open({ enter = true, auto_close = true }) end, { desc = "Test: output" })
    vim.keymap.set("n", "<leader>tS", function() nt.summary.toggle() end, { desc = "Test: summary" })
    vim.keymap.set("n", "<leader>ta", function() nt.run.run({ suite = true }) end, { desc = "Test: all" })
  end,
}
